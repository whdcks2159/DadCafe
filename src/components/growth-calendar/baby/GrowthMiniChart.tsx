...
export default function GrowthMiniChart({ logs, currentMonth }: GrowthMiniChartProps) {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 mb-4 border border-neutral-100">
      <div className="text-sm font-bold text-neutral-700 mb-3">성장 기록</div>

      {withHeight.length >= 2 && (
        <div className="mb-3">
          <div className="text-xs text-neutral-400 mb-1.5">키 (cm)</div>
            ...
            {withHeight.map((log) => {
              ...
              return (
                <div key={log.monthNumber} className="flex-1 flex flex-col items-center gap-0.5">
                  <div className="text-xs text-neutral-400 font-medium">{log.heightCm}</div>
                  <div
                  ...
                  />
                  <div className="text-xs text-neutral-400">{log.monthNumber}m</div>
                </div>
              );
            })}
          </div>
      ...
      {withWeight.length >= 2 && (
        <div>
          <div className="text-xs text-neutral-400 mb-1.5">몸무게 (kg)</div>
          <div className="flex items-end gap-1.5 h-14">
            {withWeight.map((log) => {
              ...
              return (
                <div key={log.monthNumber} className="flex-1 flex flex-col items-center gap-0.5">
                  <div className="text-xs text-neutral-400 font-medium">{log.weightKg}</div>
                  <div
                    className={`w-full rounded-t-md transition-all ${
